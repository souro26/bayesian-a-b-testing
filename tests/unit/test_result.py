...
def make_decision(joint=True, composite=True, guardrail_fail=False, notes=None):
    ...
    if joint:
        joint_obj = NS(
            ...
            condition_probs={"B": {"primary": 0.9, "guardrail": 0.7}, "C": {"primary": 0.4, "guardrail": 0.3}},
...
def make_hierarchical_results(violations=False):
    ...
    segment_results = {
        "mobile":  make_segment_decision(state="strong win",  recommendation="ship variant"),
        "desktop": make_segment_decision(state="weak win",    recommendation="consider shipping"),
        "tv":      make_segment_decision(state="inconclusive", recommendation="continue experiment",
...
class TestHierarchicalResults:
    ...
    def test_segment_summary_detects_conflict(self):
        ...
        segment_results = {
            "mobile":  make_segment_decision(state="strong win",   recommendation="ship variant"),
            "desktop": make_segment_decision(state="inconclusive", recommendation="do not ship"),
        }
    ...
    def test_segment_summary_consistent_winner_message(self):
        """segment_summary reports consistent winner when all segments agree."""
        import io, sys
        r = make_hierarchical_results()
        buf = io.StringIO()
        sys.stdout = buf
    ...
    def test_segment_results_not_affected_by_flat_path(self):
        """Flat Results object has no segment_results attribute side effects."""
