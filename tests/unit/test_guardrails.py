import numpy as np
import pytest
from argonx.decision_rules.guardrails import compute_guardrail, compute_all_guardrails, _validate_guardrail_inputs
...
class TestComputeGuardrail:
    ...
    def test_clear_degradation(self):
        ...
        res = compute_guardrail(samples, ['A', 'B'], control='A', metric='latency', threshold=0.5, lower_is_better=True)
    ...
    def test_lower_is_better_false(self):
        ...
        samples = np.column_stack([control, variant])
        res = compute_guardrail(samples, ['A', 'B'], control='A', metric='revenue', threshold=0.5, lower_is_better=False)
        assert res[0].prob_degraded == 1.0
    ...
    def test_expected_degradation_positive(self):
        ...
        samples = np.column_stack([control, variant])
        res = compute_guardrail(samples, ['A', 'B'], control='A', metric='latency', threshold=0.5)
    ...
    def test_severity_levels(self):
        ...
        samples = np.column_stack([control, variant])
        res = compute_guardrail(samples, ['A', 'B'], control='A', metric='latency', threshold=0.9)
...
class TestComputeAllGuardrails:
    ...
    def test_empty_guardrails(self):
        ...
        samples = np.random.rand(10, 2)
        res = compute_all_guardrails(guardrail_samples={}, variant_names=['A', 'B'], control='A', thresholds={}, primary_passed=True)
    ...
    def test_missing_threshold_raises(self):
        ...
        samples = np.random.rand(10, 2)
        with pytest.raises(ValueError):
            compute_all_guardrails(guardrail_samples={'latency': samples}, variant_names=['A', 'B'], control='A', thresholds={}, primary_passed=True)
    ...
    def test_conflict_detection(self):
        ...
        variant = np.array([2, 2, 2])
        samples = np.column_stack([control, variant])
        res = compute_all_guardrails(guardrail_samples={'latency': samples}, variant_names=['A', 'B'], control='A', thresholds={'latency': 0.5}, primary_passed=True)
    ...
    def test_no_conflict_when_primary_fails(self):
        ...
        variant = np.array([2, 2, 2])
        samples = np.column_stack([control, variant])
        res = compute_all_guardrails(guardrail_samples={'latency': samples}, variant_names=['A', 'B'], control='A', thresholds={'latency': 0.5}, primary_passed=False)
