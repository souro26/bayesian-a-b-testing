...
def make_df_lognormal(lift_b=0.08, n=N):
    return pd.DataFrame({'variant': ['control'] * n + ['variant_b'] * n, 'revenue': np.concatenate([np.random.lognormal(3.8, 0.6, n), np.random.lognormal(3.8 + lift_b, 0.6, n)]), 'page_load': np.concatenate([np.abs(np.random.normal(1.2, 0.1, n)) + 0.5, np.abs(np.random.normal(1.1, 0.1, n)) + 0.5])})

def make_df_binary(n=N):
    return pd.DataFrame({'variant': ['control'] * n + ['variant_b'] * n, 'converted': np.concatenate([np.random.binomial(1, 0.1, n), np.random.binomial(1, 0.14, n)])})
...
def make_df_segmented(n_per_segment=150):
    ...
    for seg in segments:
        ...
        for variant, mu in [('control', 3.8), ('variant_b', 3.88)]:
            ...
            rows.append(pd.DataFrame({'variant': [variant] * n, 'device_type': [seg] * n, 'revenue': revenue, 'page_load': np.abs(np.random.normal(1.2, 0.1, n)) + 0.5}))
...
class TestInputValidation:
    ...
    def test_rejects_lower_is_better_not_in_guardrails(self):
        ...
        df = make_df_lognormal()
        with pytest.raises(ValueError):
            Experiment(df, variant_col='variant', primary_metric='revenue', model='lognormal', guardrails=['page_load'], lower_is_better={'revenue': True})
    ...
    def test_primary_metric_callable_accepted(self):
        ...
        df = make_df_lognormal()
        Experiment(df, variant_col='variant', primary_metric=lambda d: d['revenue'], model='lognormal')
...
class TestMetricResolution:
    ...
    def test_lambda_metric_runs(self):
        ...
        df = make_df_lognormal()
        exp = Experiment(df, 'variant', primary_metric=lambda d: d['revenue'], model='lognormal', control='control')
        result = exp.run(n_draws=200)
        assert result.best_variant is not None
...
class TestFlatModelSelection:
    ...
    def test_gaussian_model_runs(self):
        ...
        df = pd.DataFrame({'variant': ['control'] * N + ['variant_b'] * N, 'latency': np.concatenate([np.random.normal(100, 10, N), np.random.normal(108, 10, N)])})
    ...
    def test_poisson_model_runs(self):
        ...
        df = pd.DataFrame({'variant': ['control'] * N + ['variant_b'] * N, 'purchases': np.concatenate([np.random.poisson(5, N).astype(int), np.random.poisson(6, N).astype(int)])})
...
class TestFlatResultStructure:
    ...
    def test_guardrail_passes_when_improving(self):
        ...
        df = make_df_lognormal()
        exp = Experiment(df, 'variant', 'revenue', 'lognormal', guardrails=['page_load'], lower_is_better={'page_load': True}, control='control')
        result = exp.run(n_draws=200, config={'guardrail_thresholds': {'page_load': 0.1}})
    ...
    def test_composite_score_computed(self):
        ...
        df = make_df_lognormal()
        exp = Experiment(df, 'variant', 'revenue', 'lognormal', guardrails=['page_load'], lower_is_better={'page_load': True}, control='control')
        result = exp.run(n_draws=200, composite_weights={'primary': 1.0, 'page_load': 0.3}, config={'guardrail_thresholds': {'page_load': 0.1}})
...
class TestHierarchicalModelSelection:
    ...
    def test_hierarchical_model_selected_with_segment_col(self):
        ...
        exp = Experiment(df, 'variant', 'revenue', 'lognormal', segment_col='device_type', control='control')
    ...
    def test_priors_accepted_without_error(self):
        ...
        df = make_df_segmented()
        exp = Experiment(df, 'variant', 'revenue', 'lognormal', segment_col='device_type', control='control', priors={'tau_prior_beta': 2.0})
        result = exp.run(n_draws=200)
        assert result is not None
    ...
    def test_hierarchical_with_guardrail(self):
        ...
        df = make_df_segmented()
        exp = Experiment(df, 'variant', 'revenue', 'lognormal', segment_col='device_type', guardrails=['page_load'], lower_is_better={'page_load': True}, control='control')
        result = exp.run(n_draws=200, config={'guardrail_thresholds': {'page_load': 0.1}})
        assert result is not None
