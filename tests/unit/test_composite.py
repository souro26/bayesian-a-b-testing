...
class TestThreshold:
    ...
    def test_prob_exceeds_threshold(self):
        ...
        primary, guardrails, names, weights, gb = make_basic_inputs()
        res = compute_composite_score(primary, guardrails, names, 'control', weights, gb, threshold=0.0)
        assert 0 <= res.prob_exceeds_threshold['B'] <= 1

    def test_gap_distribution_centered(self):
        """Gap distribution shifts by threshold."""
        primary, guardrails, names, weights, gb = make_basic_inputs()
        res = compute_composite_score(primary, guardrails, names, 'control', weights, gb, threshold=0.1)
    ...
    def test_gap_hdi_positive_when_far_above_threshold(self):
        ...
        weights = {'primary': 1.0}
        gb = MockGuardrails({'control': True, 'B': True})
        res = compute_composite_score(primary, guardrails, names, 'control', weights, gb, threshold=0.5)
        low, _ = res.gap_hdi['B']
        assert low > 0
...
class TestGuardrails:
    ...
    def test_penalty_applied_when_failed(self):
        ...
        res = compute_composite_score(primary, guardrails, names, 'control', weights, gb, guardrail_penalty=1.0)
        assert res.score['B'] < 0
    ...
    def test_penalty_zero_when_all_pass(self):
        ...
        primary, guardrails, names, weights, gb = make_basic_inputs()
        res = compute_composite_score(primary, guardrails, names, 'control', weights, gb, guardrail_penalty=10.0)
        assert res.score['B'] > 0
...
class TestWeights:
    ...
    def test_invalid_weight_keys_raises(self):
        ...
        primary, guardrails, names, _, gb = make_basic_inputs()
        with pytest.raises(ValueError):
            compute_composite_score(primary, guardrails, names, 'control', {'invalid': 1.0}, gb)
    ...
    def test_asymmetric_deterioration_weights(self):
        ...
        weights = {'primary': 1.0}
        gb = MockGuardrails({'control': True, 'B': True})
        res_sym = compute_composite_score(primary, guardrails, names, 'control', weights, gb)
        res_asym = compute_composite_score(primary, guardrails, names, 'control', weights, gb, deterioration_weights={'primary': 2.0})
...
class TestEdgeCases:
    ...
    def test_all_variants_below_threshold(self):
        ...
        primary, guardrails, names, weights, gb = make_basic_inputs()
        res = compute_composite_score(primary, guardrails, names, 'control', weights, gb, threshold=100.0)
